...
class MainWindow(QMainWindow):
    ...
    def check_for_dependencies(self):
        ...
        """
        # FFmpeg (audio/video) is required on all platforms.
        ...
        # Document conversion:
        # - Linux: uses unoconv (calls LibreOffice via UNO)
        # - Windows: use LibreOffice directly via 'soffice(.com) --headless --convert-to'
        self.unoconv = utils.is_installed('unoconv')
        self.soffice = utils.is_installed('soffice.com') or utils.is_installed('soffice')

        if sys.platform.startswith('win'):
            self.doc_converter = 'soffice'
        else:
            self.doc_converter = 'unoconv' if self.unoconv else 'soffice'

        # Image conversion:
        # - Linux: 'convert' (ImageMagick 6/7), or 'magick' (IM7)
        # - Windows: MUST use 'magick' (Windows has its own convert.exe)
        if sys.platform.startswith('win'):
            self.imagemagick = utils.is_installed('magick')
        else:
            self.imagemagick = utils.is_installed('convert') or utils.is_installed('magick')

        missing = []
        if not self.ffmpeg_path:
            missing.append('ffmpeg')

        if self.doc_converter == 'unoconv':
            if not self.unoconv:
        ...
        if missing:
            status = self.tr('Missing dependencies: ') + ', '.join(missing)
        else:
            status = self.tr('All dependencies are installed.')
    ...
    def open_dialog_about(self):
        ...
        text = '''<b> FF Multi Converter {0} </b>
                 ...
                 <p>Copyright &copy; 2026 {3}
                 (<a href="mailto:{4}">{4}</a>)
                 <p>Copyright &copy; 2023 {5}
                 <p>Copyright &copy; 2011-2016 {6}
                 <br>License: {7}
                 <p>Python {8} - Qt {9} - PyQt {10} on {11}'''\
                 .format(ffmc.__version__, msg, ffmc.__url__,
                         ffmc.__windows_port_author__,
                         ffmc.__windows_port_author_email__,
                         ffmc.__author2__,
                         ffmc.__author__,
