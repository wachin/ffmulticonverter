...
def is_installed(program):
    """
    Return an absolute path to *program* if it can be executed, else ''.

    - If *program* is an absolute/relative path, we verify it exists.
    - Otherwise we search PATH (cross-platform, honoring PATHEXT on Windows).
    """
    program = os.path.expanduser(program)

    # If user passed a direct path, trust it if it exists.
...
def start_office_listener():
    ...
    The original Linux version used `unoconv --listener` (UNO listener).
    On Windows we convert documents by calling LibreOffice in headless mode
    via `soffice --headless --convert-to`, so no persistent listener is needed.

    This function is kept to avoid import/runtime errors.
    """
    return None
