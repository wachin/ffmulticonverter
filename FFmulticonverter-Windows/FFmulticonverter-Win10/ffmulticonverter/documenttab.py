...
class DocumentTab(QWidget):
    ...
    def ok_to_continue(self):
        ...
        On Linux this app historically uses **unoconv**.
        On Windows, unoconv is usually not available, so we use LibreOffice
        directly via **soffice --headless --convert-to**.
        ...
        if getattr(self.parent, 'doc_converter', 'unoconv') == 'unoconv':
            ...
            if not getattr(self.parent, 'soffice', None):
                QMessageBox.warning(self, 'FF Multi Converter - ' + self.tr('Error!'),
