...
def _decode_process_output(data):
    """Decode command output using the local Windows encoding when needed."""
    if not data:
        return ''
...
class Progress(QDialog):
    ...
    def reject(self):
        ...
        On POSIX, pause audio/video conversion while waiting for user response.
        """
        if not self.files:
            QDialog.accept(self)
            return
        can_pause = hasattr(signal, 'SIGSTOP') and hasattr(signal, 'SIGCONT')
        if self._type == 'AudioVideo' and can_pause:
    ...
    def convert_image(self, from_file, to_file, size, mntaspect, imgcmd):
        ...
        final_output = _decode_process_output(child.stdout.read())
